  ...
  return (
    <div className="bg-white p-4 sm:p-6 lg:p-10 shadow-md border border-slate-200 space-y-6 sm:space-y-8 text-base">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4 sm:gap-6">
        {FODA_FIELDS.map((f) => (
          <div key={f.key} className={`${f.bg} p-4 sm:p-6 border-2 ${f.border} min-h-[140px] sm:min-h-[200px] flex flex-col`}>
            <h3 className={`font-black uppercase mb-3 sm:mb-4 ${f.text}`}>{f.titulo}</h3>
            <textarea
              value={foda[f.key]}
              onChange={(e) => setFodaCampo(f.key, e.target.value)}
              placeholder={f.placeholder}
              className="flex-1 bg-transparent border-none resize-none focus:ring-0 outline-none text-base"
            />
          </div>
        ))}
      </div>

      <div className="border-t-2 border-slate-200 pt-6 sm:pt-10 mt-6 sm:mt-10">
        <h3 className="text-xl sm:text-2xl font-bold text-emerald-800 mb-4 sm:mb-6">Cruces Estratégicos (Familias de Problemas)</h3>
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4 sm:gap-6">
          {CRUCES_FIELDS.map((c) => (
            <div key={c.key} className={`${c.bg} p-4 sm:p-6 border border-slate-200 shadow-sm`}>
              <h4 className={`font-black text-sm mb-1 uppercase tracking-wider ${c.color}`}>{c.titulo}</h4>
              <p className={`text-xs mb-3 sm:mb-4 italic ${c.textHint}`}>{c.hint}</p>
              <textarea
                value={cruces[c.key]}
                onChange={(e) => setCruceCampo(c.key, e.target.value)}
                rows={3}
                placeholder={c.placeholder}
                className={`w-full bg-white border ${c.borderInput} p-3 sm:p-4 resize-none focus:ring-2 ${c.ring} outline-none text-base`}
              />
            </div>
          ))}
        </div>
